    ...
    <div className="flex flex-col gap-14">
      <section className="border-b pb-10">
        <div className="grid items-end gap-8 lg:grid-cols-[1fr_320px]">
          <div>
            ...
            <p className="mt-5 max-w-xl text-pretty text-lg text-muted-foreground">
              Browse the catalog by category, find exactly what you need, and check out in minutes.
            </p>
          ...
          <dl className="border bg-card">
            <p className="label-caps border-b bg-secondary px-4 py-2 text-foreground">Catalog sheet</p>
        ...
        <div aria-hidden className="mt-10 flex h-9 w-full max-w-md text-foreground/60">
          ...
          <p className="label-caps ml-3 self-center whitespace-nowrap">SF · General catalog</p>
        </div>
      </section>
        ...
        {categories.isPending ? (
          ...
          <div className="-mx-1 flex flex-wrap gap-2 px-1">
            {categories.data.map((category) => (
              <Link key={category.public_id} href={`/categories/${category.public_id}`} className="rack-sign">
                {category.name}
                <ArrowRight
                  aria-hidden
      ...
      <section aria-labelledby="home-latest" className="flex flex-col gap-4">
        ...
        {latest.isPending ? (
          ...
          <EmptyState title="No products yet" description="Products will appear here as soon as they're published." />
        ) : (
