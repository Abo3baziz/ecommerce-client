...
import { catalogHasActiveFilters, catalogQueryParams, useCatalogFilters } from "@/features/catalog/filters";
...
function CategoryProductsBrowser({ categoryId }: { categoryId: string }) {
  ...
  const productsQuery = useCategoryProducts(categoryId, catalogQueryParams(controller.values));
...
export function CategoryClient() {
  ...
  if (categoryQuery.isError) {
    ...
    return <ErrorState error={categoryQuery.error} onRetry={() => void categoryQuery.refetch()} />;
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      <header className="flex flex-col gap-2">
        ...
        {category.description ? <p className="max-w-3xl text-sm text-muted-foreground">{category.description}</p> : null}
      </header>
